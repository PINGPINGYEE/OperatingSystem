...
double array[SIZE];         // 입력 데이터를 저장할 배열
...
// 스레드가 실행할 함수, 배열의 지정된 부분에 sin 함수를 적용
...
struct timeval do_single_thread() {
    ...
    struct ThreadData data = {0, SIZE};
    for (int i = 0; i < SIZE; i++) {
        single_thread_result[i] = sin(array[i]); // 각 요소에 대해 sin 계산
    }
...
struct timeval do_multi_thread() {
    ...
    // 배열을 스레드 수에 따라 나누고 각 스레드에 작업 할당
...
void verify_results() {
    for (int i = 0; i < SIZE; i++) {
        if (fabs(single_thread_result[i] - array[i]) > 1e-9) { // Consider precision issues in floating-point comparison
...
int main(int argc, char* argv[]) {
    ...
    // single_thread_result로부터 array에 다시 데이터를 채웁니다.
    for (int i = 0; i < SIZE; i++) {
        array[i] = i * 0.1; // 배열을 원래 값으로 재설정합니다.
    }
